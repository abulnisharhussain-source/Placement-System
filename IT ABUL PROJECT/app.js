...
window.showForgotPasswordModal = function() {
  const bodyHTML = `
    <form id="forgot-pw-form" onsubmit="handleResetPassword(event); return false;" style="display:flex; flex-direction:column; gap:16px">
      ...
      <div class="form-group">
        <label style="font-size:.8rem; font-weight:600; margin-bottom:6px; display:block">Email Address *</label>
        <div class="input-wrapper">
          <i class="ph ph-envelope"></i>
          <input type="email" id="forgot-email" placeholder="your@email.com" required style="width:100%">
        </div>
      </div>

      <div class="form-group">
        <label style="font-size:.8rem; font-weight:600; margin-bottom:6px; display:block">New Password *</label>
        <div class="input-wrapper">
          <i class="ph ph-lock"></i>
          <input type="password" id="forgot-new-pass" placeholder="Min 6 characters" required style="width:100%">
        </div>
      </div>

      <div class="form-group">
        <label style="font-size:.8rem; font-weight:600; margin-bottom:6px; display:block">Confirm New Password *</label>
        <div class="input-wrapper">
          <i class="ph ph-lock-key"></i>
          <input type="password" id="forgot-confirm-pass" placeholder="Repeat new password" required style="width:100%">
        </div>
      </div>

      <div style="display:flex; justify-content:flex-end; gap:12px; margin-top:8px">
        <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
        <button type="submit" class="btn btn-primary"><i class="ph ph-key"></i> Reset Password</button>
      </div>
    </form>
  `;
  openModal('🔑 Reset Account Password', bodyHTML);
};
...
window.handleResetPassword = function(e) {
  ...
  if (!email || !newPass) {
    toast('Please enter email and new password', 'error');
    return false;
  }
  if (newPass.length < 6) {
  ...
  if (!user) {
    toast('No account found with this email address', 'error');
    return false;
  }

  // Update user password in local store
  store.update('users', user.id, { password: newPass });

  toast('Password updated successfully! You can now sign in.', 'success');
  closeModal();

  // Auto-fill login fields with updated credentials
